...
# OSLWL_QUERY_TEST_SET.zip 	https://drive.google.com/file/d/1_xwAbI0-ri0e-PTMBtiiodifCEsOkFq3/view?usp=sharing
wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1_xwAbI0-ri0e-PTMBtiiodifCEsOkFq3' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1_xwAbI0-ri0e-PTMBtiiodifCEsOkFq3" -O OSLWL/OSLWL_QUERY_TEST_SET.zip && rm -rf /tmp/cookies.txt
